...
DATABASE_PATH = r"best-ebus\scenario\eBuS\database\eBuS.duckdb"
OUTPUT_DIR = r"C:\Users\svens\Nextcloud\Masterarbeit\Grafiken\NewPlots"
...
class EnergyConsumptionAnalysis:
    ...
    @staticmethod
    def energy_over_distance_scatter(output_dir=OUTPUT_DIR):
        con = duckdb.connect(OUTPUT_DIR)
        df = con.execute("""
            SELECT
                battery_totalEnergyConsumed,
    ...
    @staticmethod
    def energy_efficiency_by_type(output_dir=OUTPUT_DIR):
    ...
    @staticmethod
    def energy_efficiency_over_avg_speed(output_dir=OUTPUT_DIR):
        con = duckdb.connect(OUTPUT_DIR)
        df = con.execute("""
            SELECT
                battery_totalEnergyConsumed,
    ...
    @staticmethod
    def energy_efficiency_over_avg_speed_stop_correllation(output_dir=OUTPUT_DIR):
        con = duckdb.connect(OUTPUT_DIR
        )
        df = con.execute("""
            SELECT
    ...
    @staticmethod
    def battery_soc_over_time(output_dir=OUTPUT_DIR):
        con = duckdb.connect(OUTPUT_DIR)
    ...
    @staticmethod
    def energy_efficiency_over_stop_count(output_dir=OUTPUT_DIR):
        con = duckdb.connect(OUTPUT_DIR)
        df = con.execute("""
            SELECT
                battery_totalEnergyConsumed,
...
if __name__ == "__main__":
    EnergyConsumptionAnalysis.total_energy_consumption()
